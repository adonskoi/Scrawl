...
def index():
    ...
    return render_template('base.html',
                           pages=pages,
                           pages_with_sub_pages=pages_with_sub_pages)
...
def render_page(page_name):
    ...
    if request.method == 'POST':
        ...
        # Purification of the content from the extra spaces
    ...
    pages_with_sub_pages = pages_utils.get_pages_info(work_directory=work_directory)
    if not page:
        # If there is no page redirect to the main page
        return redirect(url_for('pages.index'))
    page_name, sub_page_name = page['finding_pages']
    return render_template(
        'base.html', pages=pages,
        current_page=page_name, current_sub_page=sub_page_name,
...
def create_page():
    ...
    work_directory = current_app.config['PAGES_PATH']
    status = pages_utils.create_page(page_name=page_name,
                                     work_directory=work_directory)
    if status:
        message = 'The page was created successfully'
    else:
        message = 'The page with this name already exists'
    print(message)
